    ...
    public:
        ...
        Color Li(const Ray &ray, Sampler &rng) override
        {
            ...
            for (int i = 0;; i++) {
                ...
                if (!its)
                {
                    obEm += m_scene->evaluateBackground(nextRay.direction).value * weight;
                    break;
                }
                obEm += (weight * its.evaluateEmission());
                if (i >= depth - 1) break;
                ...
                if (m_scene->hasLights()) {
                    ...
                    if (!lightSample.light->canBeIntersected()) {
                        ...
                        if (isVisible) {
                            ...
                            obEm += ((weight * bsdfEval.value * directSample.weight) / lightSample.probability);
                        }
                    }
                } 
