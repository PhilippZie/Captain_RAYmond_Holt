...
public:
    ...
    friend Color max(const Color &a, const Color &b) { BUILD(std::max(a[i], b[i])) }
    /// @brief Returns the component-wise minimum of two colors.
    friend Color min(const Color &a, const Color &b) { BUILD(std::min(a[i], b[i])) }
    ...
    Color operator*=(const float &other) { return *this = *this * other; }
