...
#define assert_nonnegative(x, ctx) if (!check_nonnegative(x, __FILE__, __LINE__)) { ctx; abort(); }
template <typename T>
inline bool check_nonnegative(const T &n, const char *file, size_t line) {
#ifdef ENABLE_ASSERTIONS
    if (!(n >= 0)) {
        logger(EError, "negative value in %s:%d (%s)", file, line, n);
        return false;
    }
#endif
    return true;
