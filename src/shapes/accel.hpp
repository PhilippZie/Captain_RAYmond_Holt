...
class AccelerationStructure : public Shape {
    ...
    /// @brief A bin used for SAH binning.
    struct Bin {
        Bounds aabb;
        int count = 0;
        std::vector<int> primitiveIndicesInBin;
    ...
    NodeIndex binning(Node &node, int splitAxis) {
        const int binCount = 16; // number of bins
        Bin* bins = new Bin[binCount]; // bins for SAH binning
        float bestCost = Infinity; // best cost initialized to almost infinity

            float minBounds = Infinity; // min value of the bounding box
            float maxBounds = -Infinity; // max value of the bounding box

            // get the minimal and maximal bounds of all primitives of the node
            for (int i = 0; i < node.primitiveCount; i++) {
                const Point centroid = getCentroid(m_primitiveIndices[node.leftFirst + i]);
                minBounds = min(minBounds, centroid[splitAxis]);
                maxBounds = max(maxBounds, centroid[splitAxis]);
            }
            

            float scale = binCount / (maxBounds - minBounds); // scale factor for the binning
            // populate the bins
            for (int i = 0; i < node.primitiveCount; i++) {
                const Point centroid = getCentroid(m_primitiveIndices[node.leftFirst + i]);
                int binIndex = min(floor((centroid[splitAxis] - minBounds) * scale), binCount - 1);
                bins[binIndex].aabb.extend(centroid);
                bins[binIndex].count++;
                bins[binIndex].primitiveIndicesInBin.push_back(m_primitiveIndices[node.leftFirst + i]);
            }
            

            float* leftArea = new float[binCount - 1]; // surface area of the left bins
            float* rightArea = new float[binCount - 1]; // surface area of the right bins
            int* leftCount = new int[binCount - 1]; // number of primitives in the left bins
            int* rightCount = new int[binCount - 1]; // number of primitives in the right bins
            Bounds leftBounds = Bounds::empty(); // bounding box of the left bins
            Bounds rightBounds = Bounds::empty(); // bounding box of the right bins
            int leftSum = 0; // sum of the primitives in the left bins
            int rightSum = 0; // sum of the primitives in the right bins
            std::vector <std::vector <int>> leftPrimitives(binCount, std::vector<int>());
            std::vector <std::vector <int>> rightPrimitives(binCount, std::vector<int>());
            ...
            for (int i = 0; i < binCount - 1; i++) {
                ...
                leftCount[i] = leftSum;
                leftBounds.extend(bins[i].aabb);
                leftArea[i] = surfaceArea(leftBounds);

                if (!bins[i].primitiveIndicesInBin.empty()) {
                    leftPrimitives[i].insert(leftPrimitives[i].end(), bins[i].primitiveIndicesInBin.begin(), bins[i].primitiveIndicesInBin.end());
                }

                rightSum += bins[binCount - 1 - i].count;
                rightCount[binCount - 2 - i] = rightSum;
                rightBounds.extend(bins[binCount - 1 - i].aabb);
                rightArea[binCount - 2 - i] = surfaceArea(rightBounds);

                if (!bins[binCount - 2 - i].primitiveIndicesInBin.empty()) {
                    rightPrimitives[binCount - 2 - i].insert(rightPrimitives[binCount - 2 - i].end(), 
                            bins[binCount - 2 - i].primitiveIndicesInBin.begin(), bins[binCount - 2 - i].primitiveIndicesInBin.end());
                }

                if (i > 0) {
                    leftPrimitives[i].insert(leftPrimitives[i].end(), leftPrimitives[i-1].begin(), leftPrimitives[i-1].end());
                }

                if ((binCount - 2 - i) < (binCount - 1)) {
                    rightPrimitives[binCount - 2 - i].insert(rightPrimitives[binCount - 2 - i].end(), rightPrimitives[binCount - 1 - i].begin(), rightPrimitives[binCount - 1 - i].end());
                }

                //sort(leftPrimitives[i].begin(), leftPrimitives[i].end());
                //sort(rightPrimitives[binCount - 2 - i].begin(), rightPrimitives[binCount - 2 - i].end());

            }

            // calculate cost for each plane
            ...
            for(int i = 0; i < binCount - 1; i++) {
                ...
                if (cost < bestCost) {
                    ...
                    firstRightIndex = rightPrimitives[i].front();
                    lastLeftIndex = leftPrimitives[binCount - 2 - i].back();
                }
            }

        
            //sort the primitve indeces
            while (firstRightIndex <= lastLeftIndex) {
                const Point centroid = getCentroid(m_primitiveIndices[firstRightIndex]);
                int binIndex = min((int)((centroid[splitAxis] - minBounds) * scale), binCount - 1);
