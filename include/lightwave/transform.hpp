...
public:
    ...
    /// @brief Transforms a normal for shading normal, transform with matrix adjoint (transpose of inverse).
    Vector applyNormal (const Vector &normal) {
        const Vector4 result = m_inverse.transpose() * Vector4(normal, 0);
        return Vector(result.x(), result.y(), result.z());
    }
